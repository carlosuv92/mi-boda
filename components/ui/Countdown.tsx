    ...
    <div className="flex justify-center gap-3 md:gap-5">
      {timeUnits.map((unit, index) => (
        ...
        >
          ...
          } rounded-lg px-3 py-2 md:px-5 md:py-3 min-w-[55px] md:min-w-[75px]`}>
            <span className={`text-xl md:text-3xl lg:text-4xl font-playfair font-semibold ${
              isDark ? 'text-white' : 'text-text-primary'
            }`}>
              {String(unit.value).padStart(2, '0')}
            </span>
          </div>
          <span className={`text-[10px] md:text-xs mt-1.5 uppercase tracking-widest ${
